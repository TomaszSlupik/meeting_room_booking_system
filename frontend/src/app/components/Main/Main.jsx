...
export default function Main() {
  ...
  const getWeekDates = () => {
    ...
    const handleLogout = () => {
      ...
      ['token', 'ally-supports-cache'].forEach(item => localStorage.removeItem(item));
    }
    ...
    <div className="wrapper_box info_reserve">
        ...
        <div className="header_booking-details">
            <div className="date_choice">
                ...
                <p className="selected-date">Wybrana data: {date.toLocaleDateString('pl-PL')}</p>
            </div>
            <div className="additional-info">
                <p><strong>Godziny dostępności:</strong><span className='additional-main'> 8:00 - 15:00 </span></p>
                <p><strong>Wybierz salę:</strong><span className='additional-main'> Masz możliwość wyboru jednej z pięciu dostępnych sal konferencyjnych. </span></p>
                <p><strong>Informacje kontaktowe:</strong> <a href="slupiktomasz@gmail.com"> slupiktomasz@gmail.com</a></p>
            </div>
        </div>
        <Button 
            variant="outlined"
        ...
        <div className='view_week'>
          ...
          <div className='view_week-name'>
            {getWeekDates().map((day, index) => (
              <div key={index} style={{ textAlign: 'center', padding: '10px', color: "#fff" }}>
                <p
                style={{
                  color: '#fff',
                  textAlign: 'left'
                }}
                >{day.toLocaleDateString()}</p>
                {isReserved(day) && <span className='view_isreserved'>Zarezerwowane</span>}
              </div>
            ))}
          </div>
        </div>
